...
// data structure represent a zip file
...
export async function zipToZipObj(zipFile: Base64String): Promise<ZipObj> {
    ...
    for (let [path, file] of files) {
        ...
        if (file.dir) {
            ...
            console.assert(
                !zipObjDirCache[path],
                'zipObjDirCache should only be access once'
            );
            ...
            zipObjDirCache[parentPath].items.push({
                name: dirName,
                content: zipObjDirCache[path]
            });
        } else {
    ...
    function buildZip(zip: JSZip, zipObj: ZipObj) {
        for (const zipItem of zipObj.items) {
            if (isZipFolderItem(zipItem)) {
                ...
                zip.file(zipItem.name, zipItem.content, {
                    base64: true,
                    binary: true
                });
            }
...
export function fixZipObj(zipObj: ZipObj): ZipObj {
